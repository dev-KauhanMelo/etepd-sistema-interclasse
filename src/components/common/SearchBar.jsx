...
export default function SearchBar({ value, onChange, placeholder = 'Buscar turma, local…', className = '', light = false }) {
  ...
  const iconColor = light ? 'text-brand-steel' : 'text-arena-text'

  return (
    <div className={`relative ${className}`}>
      <SearchIcon className={`w-[18px] h-[18px] absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none ${iconColor}`} />
      <input
        type="search"
        value={value}
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className={`w-full cut-corner-sm border pl-10 pr-9 py-2 text-sm font-bracket font-semibold focus:outline-none [&::-webkit-search-cancel-button]:appearance-none ${skin}`}
      />
      {value && (
        <button
          type="button"
